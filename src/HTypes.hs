...
ppClause _p (HClause f ps e) =
  text (prHSymbolOp f)
    <+> sep
      [ sep (map (ppPat 10) ps) <+> text "=",
        nest 2 $ ppExpr 0 e
      ]
...
ppExpr p (HELam ps e) =
  pparens (p > 0) $
    sep
      [ text "\\" <+> sep (map (ppPat 10) ps) <+> text "->",
        ppExpr 0 e
      ]
ppExpr p (HEApply (HEApply (HEVar f@(c : _)) a1) a2)
  | not (isAlphaNum c) =
...
ppExpr p (HECase s alts) =
  pparens (p > 0) $
    (text "case" <+> ppExpr 0 s <+> text "of")
      $$ vcat (map ppAlt alts)
  where
    ppAlt (pp, e) = ppPat 0 pp <+> text "->" <+> ppExpr 0 e
...
fixSillyAt = fixAt []
  where
    fixAt s (HELam ps e) = HELam ps' (fixAt (concat ss ++ s) e) where (ps', ss) = unzip $ map findSilly ps
...
hECase se alts@((_, HELam ops _) : _) | m > 0 = HELam (take m ops) $ hECase se alts'
  where
    ...
    isPVar _ = False
    alts' =
      [ let (ps1, ps2) = splitAt m ps in (cps, hELam ps2 $ hESubst (zipWith (\(HPVar v) n -> (v, n)) ps1 ns) e)
        | (cps, HELam ps e) <- alts
      ]
...
etaReduce expr = fst $ eta expr
  where
    eta (HELam [HPVar v] (HEApply f (HEVar v'))) | v == v' && v `notElem` vs = (f', vs)
      where
        (f', vs) = eta f
